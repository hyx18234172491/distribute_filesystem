...
int main(int argc, char *argv[]) {
    ...
    // totals
    s.num_inodes = num_inodes;// inode数量，80
    s.num_data = num_data;  // data数量

    // inode bitmap
    int bits_per_block = (8 * UFS_BLOCK_SIZE); // remember, there are 8 bits per byte
    // 每个block的bit的数量

    s.inode_bitmap_addr = 1;    // 地址为1
    s.inode_bitmap_len = num_inodes / bits_per_block;   // inode的bitmap的长度
    if (num_inodes % bits_per_block != 0)
	s.inode_bitmap_len++;   // inode_bitmap_len现在等于1

    // data bitmap
    s.data_bitmap_addr = s.inode_bitmap_addr + s.inode_bitmap_len;  // 地址为2
    s.data_bitmap_len = num_data / bits_per_block;
    if (num_data % bits_per_block != 0)
	s.data_bitmap_len++;    // data的长度

    // inode table
    s.inode_region_addr = s.data_bitmap_addr + s.data_bitmap_len;
    int total_inode_bytes = num_inodes * sizeof(inode_t);   // inode占的字节数
    s.inode_region_len = total_inode_bytes / UFS_BLOCK_SIZE;    // inode占的block数
